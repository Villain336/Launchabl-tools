...
- **Deployment**: env vars needed — `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_PRO_MONTHLY`, `STRIPE_PRICE_PRO_YEARLY`, and (when ready) `PAYWALL_ENFORCED_TOOLS`. Until `STRIPE_SECRET_KEY` is set, the billing routes 503 and `checkEntitlement` still works in dark-launch mode (trials and shadow metrics keep accumulating even with billing unconfigured).
