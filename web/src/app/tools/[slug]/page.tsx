...
import { toolCatalog } from "@/lib/marketplace/catalog";
...
export async function generateMetadata({
  ...
  return {
    ...
    robots: tool.status === "coming-soon" || toolCatalog(tool.slug) !== "public" ? { index: false, follow: true } : undefined,
  };
}
...
function AboutCopy({ slug }: { slug: string }) {
  const copy: Record<string, string> = {
    ...
    "watermark-remover":
      "Sometimes you need to clean up a watermark you added yourself — an old proof stamp, a placeholder mark from an earlier draft, a date stamp from a camera. Box the mark, choose smooth fill (a diffusion inpaint that pulls surrounding colour into the area), blur or pixelate, and download the repainted image — your file never leaves the browser. The tool is intentionally gated behind an ownership attestation so it can't be used to strip protection from content you don't have rights to edit.",
    "file-converter":
