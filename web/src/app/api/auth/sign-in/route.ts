...
import { emailCodesEnabled, isValidEmail, issueCode, normalizeEmail, sendCodeEmail, sessionCookie, upsertUser, verifyCode } from "@/lib/auth/session";
...
export async function POST(request: NextRequest) {
  const body = (await request.json().catch(() => null)) as { email?: unknown; name?: unknown; code?: unknown } | null;
  ...
  const { user, created } = await upsertUser(email, name);
