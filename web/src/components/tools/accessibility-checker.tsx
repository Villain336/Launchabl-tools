...
import type { A11yResult } from "@/lib/accessibility-audit";
    ...
    <div>
      <div className="flex gap-2">
        ...
        <Button onClick={run} disabled={busy}>
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Check page"}
        </Button>
      </div>
      ...
      {result && (
        <div className="mt-8">
          <div className="flex flex-col items-center gap-4 rounded-2xl bg-muted p-6 sm:flex-row sm:justify-between">
            <div>
              <p className="text-sm text-muted-foreground">Accessibility scan for</p>
