...
import {
  artifactRenderers,
  type ToolPart,
} from "@/components/tools/chat/artifact-registry";
...
/** The project's white-label settings, resolved server-side for the page. */
export type ReportBrand = Pick<Project, "brand" | "name" | "company"> | null;
...
export function ReportView({
  ...
  const deliverables = report.items.filter(
    (item) => item.kind === "tool",
  ).length;
  const date = formatDate(report.createdAt);
  const agency = brand?.brand.agencyName?.trim() || null;
  const whiteLabel = Boolean(brand?.brand.hideBadge);
  const accent = brand?.brand.primary ?? null;
  const client = brand?.company?.trim() || brand?.name?.trim() || null;
  ...
  return (
    <ArtifactSessionProvider slug={report.slug}>
      <article
        ...
        data-white-label={whiteLabel || undefined}
        style={
          accent
            ? ({
                "--primary": accent,
      ...
      >
        <header className="border-b border-line pb-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="inline-flex items-center gap-2 text-[12px] font-medium tracking-wide text-ink-3 uppercase">
              {brand?.brand.logoUrl ? (
                // eslint-disable-next-line @next/next/no-img-element -- client-supplied logo from an arbitrary host
                <img
              ...
              {agency ? `${agency} · Client report` : "Client report"}
            </span>
          ...
          <h1 className="mt-4 text-[26px] leading-tight font-semibold tracking-tight text-ink sm:text-[32px]">
            {report.title}
          </h1>
          <p className="mt-2 text-[13.5px] text-ink-3">
            {client ? `Prepared for ${client} · ` : ""}
            {report.preparedBy
              ? `Prepared by ${report.preparedBy}${agency ? `, ${agency}` : ""} · `
              : ""}
            {date} · {deliverables}{" "}
          ...
          {report.trimmed && (
            <p className="mt-2 text-[12.5px] text-ink-3">
              Some generated images were left out to keep this report small; the
          ...
          {report.items.map((item, index) => {
            if (item.kind === "prompt") {
              return (
                <section
                  key={index}
                  className="rounded-[12px] border border-line bg-field/60 px-4 py-3"
                >
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="max-w-md">
                ...
                <p className="mt-1 text-[13.5px] leading-relaxed text-ink-2">
                  Launchabl&apos;s agency plan is {price} one time for unlimited
                  brand, website, content and SEO requests. No retainer. The
                  tools stay free either way.
                </p>
              </div>
        ...
        <p className="mt-4 text-center text-[11.5px] text-ink-3 print:hidden">
          AI-assisted output. Check claims before you publish.
        </p>
      </article>
